...
$pag = new Pagination(['totalCount' => User::find()->count()]);
$this->title = 'Users';
...
<div class="user-index">
    ...
    <div id="w0" class="grid-view">
        <table class="table table-striped table-bordered">
            ...
            <tr>
                <th>#</th>
                <th><a href="/user/index?sort=id" data-sort="id">ID</a></th>
                <th><a href="/user/index?sort=username" data-sort="username">Username</a></th>
                <th>Email</th>
                <th>Status</th>
                <th>Created at</th>
...
<script>
    let datas;
    $(document).ready(function loadData(page) {
        $.ajax({
            url: 'http://yiitask2back:81/api/users?page=' + page,
            headers: {
            ...
            $(document).on('click', '#next-page', function() {
                let currentPage = datas.pages.currentPage;
                if (currentPage < datas.pages.pageCount) {
    ...
    $(document).ready(function () {
        $(document).on('click', '.item-activate', function () {
            ...
            $.ajax({
                url: 'http://yiitask2back:81/api/user/activate?id=' + dataIdValue,
                headers: {
                    'Access-Control-Allow-Origin': '*',
                    'Authorization': 'Bearer ' + Cookies.get('token')
                },
                ...
                success: function (data) {
                    window.location.href = "http://yiitask2front:81/user";
                }
            })
            return false
        })
        ...
        $(document).on('click', '.item-ban', function () {
            let dataIdValue = $(this).data('key');
            $.ajax({
                url: 'http://yiitask2back:81/api/user/ban?id=' + dataIdValue,
                headers: {
                    'Access-Control-Allow-Origin': '*',
                    'Authorization': 'Bearer ' + Cookies.get('token')
                },
